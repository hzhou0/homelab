...
```sh
helm dependency build agents/platform
helm install homelab-platform agents/platform -n kyverno --create-namespace
```
...
> helm upgrade homelab-platform agents/platform -n kyverno \
...
```sh
./agents/platform/gen-kubeconfig.sh            # 1-year token -> operator.kubeconfig
./agents/platform/gen-kubeconfig.sh -d 0       # non-expiring bound-Secret token
```
...
`agents/operator/` and is out of scope for this chart; this chart delivers its **identity,
