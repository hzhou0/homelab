...
async fn admin(h: &Harness, path: &str) -> (u16, String) {
...
async fn a_serving_hypha_is_live_and_ready() {
    ...
    assert_eq!(admin(&h, "/healthz").await.0, 200);
    assert_eq!(admin(&h, "/readyz").await.0, 200);
    assert_eq!(admin(&h, "/nope").await.0, 404);
}
...
async fn client_traffic_reaches_the_metrics_endpoint() {
    ...
    let (status, body) = admin(&h, "/metrics").await;
